...
--fk 제약을 dept_test.deptno 컬럼을 참조하도록 생성하였기때문에
--1번 이외의 부서번호는 emp_test 테이블에 입력 될 수 없다.

--emp_test fk 테이스 insert
...
CREATE TABLE dept_test(
    deptno NUMBER(2), /* PRIMARY KEY --> UNIAUE 제약x --> 인덱스생성x */
    dname VARCHAR2(14),
    loc VARCHAR2(13)
);
...
--dept_test테이블의 deptno값을 참조하는 데이터가 있을 경우 삭제가 불가능
...
--FK제약 default 옵션시에는 부모 테이블의 데이터를 삭제하기전에 자식 테이블에서 참조하는 데이터가 없어야 정상적으로 삭제가 가능했음
...
--CHECK 제약 : 컬럼의 값을 정해진 범위, 혹은 ㄱ밧만 들어오게끔 제약
DROP TABLE emp_test;
CREATE TABLE emp_test(
    empno NUMBER(4),
    ename VARCHAR2(10),
...
--FOREIGN KEY 추가
--실습1. dept 테이블의 deptno컬럼으로 PRIMRY KEY 제약을 테이블 변경 DDL을 통해 생성
ALTER TABLE dept ADD CONSTRAINT pk_dept PRIMARY KEY(deptno);
--실습2. emp 테이블의 empno컬럼으로 PRIMARY KEY 제약을 테이블 변경 DDL을 통해 성성
ALTER TABLE emp ADD CONSTRAINT pk_emp PRIMARY KEY(empno);
--실습3. emp 테이블의 deptno컬럼으로 dept 테이블의 deptno 컬럼을 참조하는 fk 제약을 테이블 변경 DDL을 통해 생성
-- emp --> dept (deptno)
